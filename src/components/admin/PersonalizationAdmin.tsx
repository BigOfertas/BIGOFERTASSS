...
const GROUPS: Array<{ id: SiteAssetDefinition["group"]; title: string; description: string }> = [
  {
    id: "top",
    title: "Carrossel acima do cabeçalho",
    description:
      "Arte promocional contínua. PC e mobile usam arquivos separados para não cortar texto importante.",
  },
  {
    id: "hero",
    title: "Banner abaixo do cabeçalho",
    description: "Banner principal da campanha. É apenas visual e não possui ação ao clicar.",
  },
  {
    id: "categories",
    title: "Monte seu pedido",
    description:
      "Uma arte vertical por card. Cada card já aponta para a categoria correta da loja.",
  },
  {
    id: "ambient",
    title: "Faixa intermediária",
    description: "Arte entre Monte seu pedido e a área do Brasileirão.",
  },
  {
    id: "brasileirao",
    title: "Banner Brasileirão",
    description:
      "Ao clicar, o cliente é levado para a seção de produtos do Brasileirão na própria página.",
  },
];
...
export function PersonalizationAdmin() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-6 text-gray-500">
              Troque as artes fixas da página inicial sem alterar código. Nos banners largos, PC e
              mobile possuem uploads separados para preservar o enquadramento.
            </p>
          </div>
        </div>
      </header>
      ...
      {GROUPS.map((group) => {
        const definitions = SITE_ASSET_DEFINITIONS.filter(
        ...
        return (
          ...
          >
            ...
            <div
              className={`mt-5 grid gap-4 ${group.id === "categories" ? "sm:grid-cols-2 xl:grid-cols-3" : "lg:grid-cols-2"}`}
            >
              {definitions.map((definition) => {
                ...
                const ratioWarning =
                  actualRatio !== null &&
                  Math.abs(actualRatio - expectedRatio) / expectedRatio > 0.04;
                ...
                return (
                  ...
                  >
                    <div
                      className={`${definition.group === "categories" ? "aspect-[2/3]" : "aspect-[16/6]"} flex items-center justify-center overflow-hidden bg-white`}
                    >
                      {asset?.url ? (
                        <img
                          src={asset.url}
                          alt={`Prévia: ${definition.label} ${definition.device}`}
                          className="h-full w-full object-contain"
                          loading="lazy"
                        />
                      ) : (
                        <div className="px-5 text-center text-sm font-semibold text-gray-400">
                          Nenhuma imagem personalizada neste slot.
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-3 border-t border-gray-200 bg-white p-4">
                      <div>
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <span className="rounded-full bg-gray-100 px-2 py-1 text-[10px] font-black uppercase tracking-wide text-gray-600">
                            {definition.device}
                          </span>
                        </div>
                        <p className="mt-2 text-xs leading-5 text-gray-600">
                          Ideal:{" "}
                          <strong>
                            {definition.width} × {definition.height} px
                          </strong>{" "}
                          · proporção aproximada{" "}
                          <strong>{ratioText(definition.width, definition.height)}</strong>
                        </p>
                        <p className="mt-1 text-xs leading-5 text-gray-500">
                          Formato: {definition.format}. JPEG também é aceito quando não houver
                          transparência.
                        </p>
                        <p className="mt-1 text-xs leading-5 text-gray-500">
                          {definition.behavior}
                      ...
                      {asset ? (
                        <div
                          className={`rounded-lg px-3 py-2 text-xs ${ratioWarning ? "bg-amber-50 text-amber-800" : "bg-emerald-50 text-emerald-800"}`}
                        >
                          Arquivo atual: {asset.widthPx ?? "?"} × {asset.heightPx ?? "?"} px
                          {formatBytes(asset.byteSize) ? ` · ${formatBytes(asset.byteSize)}` : ""}.
                          {ratioWarning
                            ? " A proporção está diferente da recomendada e pode sofrer recorte."
                            : " Proporção adequada ao slot."}
                        </div>
                      ) : null}

                      <div className="flex flex-wrap gap-2">
                        <label
                          className={`inline-flex h-10 cursor-pointer items-center justify-center rounded-lg px-4 text-xs font-black ${busy ? "cursor-not-allowed bg-gray-200 text-gray-500" : "bg-red-600 text-white hover:bg-red-700"}`}
                        >
                          {busy ? (
                          ...
                          <input
                            ...
                            onChange={(event) => void handleFile(definition, event)}
                            className="sr-only"
                          />
                        </label>
                        ...
                        {asset ? (
                          <button
                            ...
                            onClick={() =>
                              void run(
                                definition.key,
                                () => clearAdminStorefrontAsset(definition.key),
                                "Imagem removida. O slot voltou ao padrão do site.",
                              )
                            }
