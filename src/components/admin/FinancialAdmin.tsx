...
function MetricCard({
  ...
  return (
    ...
    >
      ...
      <p className={`mt-3 text-xs leading-5 ${accent ? "text-red-700/80" : "text-gray-500"}`}>
        {hint}
      </p>
    </article>
  );
      ...
      <table className="min-w-[1040px] w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-gray-100">
          {rows.map((row) => (
            <tr
              key={`${mode}-${row.id}`}
              className="align-top transition hover:bg-gray-50/70 motion-reduce:transition-none"
            >
              <td className="px-5 py-4">
                ...
                <details className="group mt-2 max-w-xl">
                  ...
                  <p className="mt-2 rounded-lg bg-gray-50 px-3 py-2 text-xs leading-5 text-gray-600">
                    Produtos {money(row.baseRevenue)} + adicionais {money(row.addOnRevenue)} −
                    descontos {money(row.discounts)} − custo dos produtos {money(row.productCost)} −
                    custo dos adicionais {money(row.addOnCost)} ={" "}
                    <strong className="text-gray-950">{money(row.profit)}</strong>.
                  </p>
                </details>
              </td>
              ...
              <td className="px-4 py-4 text-right font-bold tabular-nums text-gray-950">
                {money(row.revenue)}
              </td>
              <td className="px-4 py-4 text-right font-semibold tabular-nums text-gray-600">
                {money(row.cost)}
              </td>
              <td className="px-4 py-4 text-right font-black tabular-nums text-gray-950">
                {money(row.profit)}
              </td>
              <td className="px-4 py-4 text-right font-bold tabular-nums text-gray-700">
                {percent(row.margin)}
              </td>
              <td className="px-5 py-4 text-right font-black tabular-nums text-red-600">
                {money(row.share5)}
              </td>
            </tr>
          ))}
    ...
    <section className="rounded-2xl border border-gray-200 bg-white">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-gray-500">
              Preço cobrado e custo vigente para novas compras.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="divide-y divide-gray-100">
        {rows.map((row) => (
          <div
            key={row.key}
            className="grid gap-3 px-5 py-4 sm:grid-cols-[minmax(180px,1fr)_180px_180px] sm:items-end sm:px-6"
          >
            <div>
            ...
            <label className="block">
              <span className="mb-1.5 block text-[11px] font-black uppercase tracking-[0.08em] text-gray-500">
                Preço
              </span>
              <div className="flex h-10 items-center rounded-lg border border-gray-200 bg-white px-3 focus-within:border-red-400 focus-within:ring-2 focus-within:ring-red-100">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({ ...current, [row.price]: event.target.value }))
                  }
                  inputMode="decimal"
            ...
            <label className="block">
              ...
              <div className="flex h-10 items-center rounded-lg border border-gray-200 bg-white px-3 focus-within:border-red-400 focus-within:ring-2 focus-within:ring-red-100">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({ ...current, [row.cost]: event.target.value }))
                  }
                  inputMode="decimal"
                  className="min-w-0 flex-1 bg-transparent text-sm font-bold text-gray-950 outline-none"
      ...
      {error ? (
        <p className="border-t border-red-100 bg-red-50 px-5 py-3 text-sm font-semibold text-red-700 sm:px-6">
          {error}
        </p>
      ) : null}
    </section>
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white">
      <div className="flex flex-col gap-4 border-b border-gray-100 px-5 py-5 sm:px-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-gray-500">
            O preço de venda continua na gestão de Produtos, que já é a fonte comercial da loja.
            Aqui você controla o custo usado nos novos snapshots financeiros.
          </p>
        </div>
        <button
          type="button"
      ...
      >
        ...
        <button
          type="submit"
          className="h-10 rounded-lg bg-gray-950 px-4 text-sm font-black text-white transition hover:bg-red-600 motion-reduce:transition-none"
        >
          Buscar
        </button>
      </form>
      ...
      {productsQuery.isLoading ? (
        <div className="px-6 py-12 text-center text-sm font-semibold text-gray-500">
          Carregando produtos...
        </div>
            ...
            <table className="min-w-[820px] w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {data.items.map((item) => (
                  <tr
                    key={item.productId}
                    className="transition hover:bg-gray-50/70 motion-reduce:transition-none"
                  >
                    <td className="px-5 py-4 sm:px-6">
                      <p className="max-w-lg font-extrabold text-gray-950">{item.name}</p>
                      <p className="mt-1 text-xs text-gray-400">
                        {categoryLabel(item.financialCategory)}
                        {item.sku ? ` • ${item.sku}` : ""}
                    ...
                    <td className="px-4 py-4">
                      <div
                        className={`flex h-10 w-36 items-center rounded-lg border px-3 focus-within:ring-2 ${item.costConfigured ? "border-gray-200 focus-within:border-red-400 focus-within:ring-red-100" : "border-amber-300 bg-amber-50 focus-within:border-amber-400 focus-within:ring-amber-100"}`}
                      >
                        <span className="mr-2 text-xs font-bold text-gray-400">R$</span>
                        <input
    ...
    return (
      <div className="rounded-2xl border border-red-200 bg-white px-6 py-12 text-center">
        ...
        <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-gray-500">
          Tente novamente em instantes. Nenhum dado da loja foi alterado.
        </p>
        <button
          type="button"
          onClick={() => void dashboardQuery.refetch()}
          className="mt-5 h-10 rounded-lg bg-red-600 px-4 text-sm font-black text-white"
        >
          Tentar novamente
        </button>
      </div>
    ...
    <div className="space-y-6">
      ...
      {data.excludedOrders > 0 ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-900">
          <strong>
            {data.excludedOrders} pedido{data.excludedOrders === 1 ? "" : "s"}
          </strong>{" "}
          pago{data.excludedOrders === 1 ? "" : "s"} no período não entra
          {data.excludedOrders === 1 ? "" : "m"} no lucro/custo porque foi
          {data.excludedOrders === 1 ? "" : "ram"} criado{data.excludedOrders === 1 ? "" : "s"} sem
          snapshot financeiro completo. Nenhum custo histórico foi inventado retroativamente.
        </div>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        <MetricCard
          ...
          hint="Produtos após descontos; frete não entra no lucro."
          icon={Banknote}
        />
        <MetricCard
          label="Lucro"
          value={money(data.summary.profit)}
          hint="Faturamento menos custos registrados nas vendas."
          icon={TrendingUp}
        />
        <MetricCard
          label="Margem"
          value={percent(data.summary.margin)}
          hint="Lucro ÷ faturamento × 100."
          icon={Percent}
        />
        <MetricCard
          label="Custos"
          value={money(data.summary.cost)}
          hint="Produtos e adicionais dos pedidos considerados."
          icon={WalletCards}
        />
        <MetricCard
          label="Pedidos"
          value={numberFormatter.format(data.summary.orders)}
          hint={`${numberFormatter.format(data.summary.units)} unidade${data.summary.units === 1 ? "" : "s"} vendida${data.summary.units === 1 ? "" : "s"}.`}
          icon={ShoppingBag}
        />
        <MetricCard
          label="Participação sobre o lucro"
          value={money(data.summary.share5)}
          hint="5% do lucro do período — nunca do faturamento."
              ...
              <AreaChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  formatter={(value, name) => [
                    money(Number(value)),
                    name === "revenue" ? "Faturamento" : name === "profit" ? "Lucro" : "Custos",
                  ]}
                  labelFormatter={(label) => `Data: ${label}`}
                />
                <Legend
                  formatter={(value) =>
                    value === "revenue" ? "Faturamento" : value === "profit" ? "Lucro" : "Custos"
                  }
                />
                <Area
                  type="monotone"
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-[0_14px_38px_rgba(15,23,42,0.04)]">
        <div className="flex flex-col gap-4 border-b border-gray-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.1em] text-gray-500">
              Desempenho
            </p>
            <h2 className="mt-1 text-lg font-black tracking-tight text-gray-950">
              Onde a loja gera resultado
            </h2>
          </div>
          <div className="inline-flex w-fit rounded-lg bg-gray-100 p-1">
            <button
              type="button"
              onClick={() => setPerformanceMode("products")}
              className={`h-8 rounded-md px-3 text-xs font-black ${performanceMode === "products" ? "bg-white text-gray-950 shadow-sm" : "text-gray-500"}`}
            >
              Produtos
            </button>
            <button
              type="button"
              onClick={() => setPerformanceMode("categories")}
              className={`h-8 rounded-md px-3 text-xs font-black ${performanceMode === "categories" ? "bg-white text-gray-950 shadow-sm" : "text-gray-500"}`}
            >
              Categorias
            </button>
          </div>
      ...
      <section className="pt-2">
        ...
        <div className="space-y-5">
          {settingsQuery.data ? (
            ...
            <div className="rounded-2xl border border-red-200 bg-white p-6 text-sm font-semibold text-red-700">
              Não foi possível carregar os valores dos adicionais.
            </div>
          )}
