...
export function ProductShippingCalculator({ productId, quantity }: { productId: string; quantity: number }) {
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-gray-50/60 p-4" aria-labelledby="product-shipping-title">
      <div className="flex items-center gap-2">
        ...
        <h2 id="product-shipping-title" className="text-sm font-black text-gray-950">Calcule a entrega</h2>
      </div>
      <p className="mt-1 text-xs leading-5 text-gray-500">Consulte o valor e a previsão total para o seu CEP antes de adicionar ao carrinho.</p>
      ...
      {result ? (
        <div className="mt-4 space-y-2">
          {result.quotes.map((quote) => (
            <div key={`${quote.carrier}-${quote.serviceId}`} className="rounded-lg border border-gray-200 bg-white px-3 py-3">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="block text-[11px] text-gray-500">Transporte: {formatTransitLabel(quote)}</span>
                </div>
          ...
          <p className="pt-1 text-[11px] leading-5 text-gray-500">
            A previsão total soma até {result.productionBusinessDays} dias úteis de preparação ao prazo de transporte informado para o CEP.
          </p>
        </div>
      ) : null}
    </section>
