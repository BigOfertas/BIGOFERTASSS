...
check(
  ...
  /ALTER TABLE public\.orders[\s\S]*ALTER COLUMN idempotency_key SET NOT NULL/.test(
    migration,
  ),
);
...
check(
  ...
  /CREATE UNIQUE INDEX IF NOT EXISTS orders_payment_provider_reference_unique/.test(
    migration,
  ) &&
...
const eventLookupPosition = migration.indexOf(
...
check(
  ...
  lockPosition >= 0 &&
    eventLookupPosition >= 0 &&
    lockPosition < eventLookupPosition,
);
...
check(
  ...
  /target_order\.paid_at IS NULL/.test(migration) &&
    /target_order\.payment_provider IS DISTINCT FROM normalized_provider/.test(
      migration,
    ) &&
    /target_order\.payment_reference IS DISTINCT FROM normalized_reference/.test(
      migration,
    ) &&
...
check(
  ...
  /REVOKE ALL ON FUNCTION public\.record_order_payment[^;]*FROM PUBLIC/.test(
    migration,
  ) &&
    /GRANT EXECUTE ON FUNCTION public\.record_order_payment[^;]*TO service_role/.test(
      migration,
    ) &&
    !/GRANT EXECUTE ON FUNCTION public\.record_order_payment[^;]*TO authenticated/.test(
      migration,
    ),
);
