...
check("admin possui seção Personalização", admin.includes('id: "personalization"') && admin.includes("<PersonalizationAdmin />"));
check("personalização contém todos os 15 slots", [
  "top_banner_desktop","top_banner_mobile","hero_desktop","hero_mobile",
  "brasileirao_banner_desktop","brasileirao_banner_mobile","ambient_banner_desktop","ambient_banner_mobile",
  "category_kids","category_training","category_shorts","category_basketball","category_windbreaker","category_fifa","category_retro",
].every((slot) => personalizationLib.includes(`"${slot}"`) && migration.includes(`'${slot}'`)));
check("admin mostra tamanho e proporção recomendados", personalization.includes("Ideal:") && personalization.includes("proporção aproximada") && personalization.includes("Formato:"));
check("carrossel superior tem dimensões PC e mobile corretas", index.includes('aspectRatio: "1920/100"') && index.includes('aspectRatio: "1080/169"'));
check("hero tem dimensões PC e mobile corretas", index.includes('aspectRatio: "1920/550"') && index.includes('aspectRatio: "1080/1067"'));
check("faixa intermediária aceita PC e mobile", index.includes("ambient_banner_desktop") && index.includes("ambient_banner_mobile"));
check("carrossel superior força movimento contínuo", promo.includes('animationPlayState: "running"') && promo.includes("animate-marquee--fast"));
...
check("cards de Monte seu pedido usam artes administráveis", categories.includes("useStorefrontPersonalization") && categories.includes("category_retro"));

check("Mais vendidos falso foi removido", !best.includes("Mais vendidos") && !best.includes("bestSellingProducts") && best.includes("Lançamentos"));
check("placeholders permanecem temporariamente", best.includes("ProductCardPlaceholder") && brazilProducts.includes("ProductCardPlaceholder"));
check("card de produto usa CTA Ver produto", productCard.includes("Ver produto") && !productCard.includes("Ver detalhes"));
check("card aceita time e tipo comercial automaticamente", productCard.includes("commercialType") && productCard.includes("time") && catalog.includes("commercial_type"));
check("catálogo inclui tipo comercial via product_purchase_settings", catalogMigration.includes("LEFT JOIN public.product_purchase_settings") && catalogMigration.includes("'commercial_type'"));
check("produto não exibe SKU operacional", !productPage.includes("SKU:") && !productPage.includes('{ label: "SKU"'));
check("produto não exibe peso e dimensões da embalagem", !productPage.includes('{ label: "Peso"') && !productPage.includes('{ label: "Dimensões"'));
...
check("catálogo possui densidade 2/3/4/5 no PC com padrão 4 e 1/2 no mobile", catalogGrid.includes("useState<DesktopColumns>(() => readStoredDesktopColumns())") && catalogGrid.includes("useState<MobileColumns>(() => readStoredMobileColumns())") && catalogGrid.includes("if (typeof window === \"undefined\") return 4") && catalogGrid.includes("if (typeof window === \"undefined\") return 2") && catalogGrid.includes("([2, 3, 4, 5] as const)") && catalogGrid.includes("([1, 2] as const)"));
...
check("funções de assets desligam verify_jwt da plataforma e validam no handler", config.includes("[functions.site-asset-presign]\nverify_jwt = false") && config.includes("[functions.site-asset-complete]\nverify_jwt = false"));
