...
function shouldKeep(product) {
  if (product.type === "camisa" || product.type === "regata") return true;
...
if (kept.length === 0) {
  throw new Error("Nenhum produto de camisa/kit infantil restou para o Mundo FIFA.");
}
...
console.log(
  `MUNDO_FIFA_SCOPE_FILTER_OK before=${report.before} kept=${report.kept} skipped=${report.skipped.length}`,
);
