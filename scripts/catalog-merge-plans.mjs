...
const warnings = duplicateNames.map(
  (name) => `Nome público repetido preservado por possuir sourceKey distinta: ${name}`,
);
const merged = {
  schemaVersion: 1,
...
if (warnings.length > 0) console.log(`RETRO_MERGE_WARNINGS count=${warnings.length}`);
