...
const BRAND_RE = /\b(ADIDAS|NIKE|PUMA|EA7|UMBRO|KAPPA|JORDAN|CASTORE|NEW BALANCE|LE COQ(?: SPORTIF)?|MACRON|MIZUNO|JOMA|UNDER ARMOUR|REEBOK)\b/i;
...
function extractEntity(title, { brand, season, model, type }) {
  ...
  if (brand) value = value.replace(new RegExp(brand.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), ' ');
...
function parseTitle(title) {
  ...
  if ((type === 'camisa' || type === 'regata') && !model) warnings.push('modelo I/II/III não identificado');
  if (version.inferred) warnings.push('versão Torcedor inferida pela ausência de Player/Jogador');
  if (audience.inferred) warnings.push('público adulto inferido pela ausência de Feminino/Kids/Infantil');
...
function mediumUrl(raw, size = 1000) {
  try {
    ...
    url.pathname = url.pathname
      .replace(/=w\d+(?:-h\d+)?[^/?#]*/i, '')
      .replace(/=s\d+[^/?#]*/i, '') + `=w${size}-h${size}-s-no-gm`;
    return url.toString();
  } catch {
    return raw;
  }
...
function buildGroupHtml(group, proposal) {
  const cards = group.images.map((image, index) => {
...
function buildOverviewHtml(items) {
  const sections = items.map(({ group, proposal, screenshot }) => {
    const images = group.images.slice(0, 5).map((image, index) => {
...
async function run() {
  ...
  if (!fs.existsSync(options.input)) throw new Error(`collector.json não encontrado: ${options.input}`);
  const report = JSON.parse(fs.readFileSync(options.input, 'utf8'));
  if (!Array.isArray(report.groups) || report.groups.length === 0) throw new Error('Nenhum grupo disponível para assimilação visual.');
  ...
  fs.writeFileSync(path.join(options.out, 'catalog-proposals.json'), JSON.stringify(products, null, 2));
  const markdown = [
    ...
    ...items.flatMap((item) => [
      ...
      `- Screenshot: ${item.screenshot}`,
      item.proposal.warnings.length ? `- Avisos: ${item.proposal.warnings.join('; ')}` : '- Avisos: nenhum',
      '',
    ]),
  ].join('\n');
  fs.writeFileSync(path.join(options.out, 'assimilation.md'), `${markdown}\n`);

  console.log(`ASSIMILATION_FINAL groups=${result.stats.groups} productImages=${result.stats.productImages} proposedBaseProducts=${result.stats.proposedBaseProducts} contactSheets=${result.stats.contactSheets} failures=${result.stats.contactSheetsWithImageFailures}`);
  for (const item of items) {
    console.log(`ASSIMILATED ${item.groupIndex}: ${item.group.title} -> entity=${item.proposal.entity || '-'} model=${item.proposal.model || '-'} version=${item.proposal.version || '-'} images=${item.group.images.length}`);
  }
  if (result.stats.contactSheetsWithImageFailures > 0) {
    throw new Error(`${result.stats.contactSheetsWithImageFailures} contact sheets tiveram imagens que não carregaram.`);
  }
}
