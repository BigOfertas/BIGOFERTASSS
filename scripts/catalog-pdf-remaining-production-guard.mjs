...
for (const group of manifest.groups ?? []) {
  if (group.mode === "club") {
    ...
    const matching = readyRows.filter(
      (row) =>
        allowed.has(normalize(row.time)) &&
        ["torcedor", "jogador", "feminino", "infantil", "calcao"].includes(row.commercial_type),
    );
    const covered = new Set(matching.map((row) => normalize(row.time)));
    const missing = (group.albums ?? [])
      .filter((album) => !covered.has(normalize(album.name)))
      .map((album) => album.name);
    if (missing.length > 0)
      throw new Error(`${group.name}: times sem produto ativo com imagem: ${missing.join(", ")}`);
    result.groups.push({
      id: group.id,
      expectedAlbums: group.albums.length,
      coveredAlbums: covered.size,
      activeProducts: matching.length,
    });
    result.totalActiveProducts += matching.length;
  } else if (group.mode === "kids") {
    const matching = readyRows.filter(
      (row) => row.commercial_type === "infantil" && normalize(row.category_slug) === "INFANTIL",
    );
    ...
    const matching = readyRows.filter(
      (row) => row.commercial_type === "calcao" && normalize(row.category_slug) === "SHORTS",
    );
    if (matching.length === 0)
      throw new Error("Shorts: nenhum produto de shorts ativo com imagem.");
    result.groups.push({ id: group.id, activeProducts: matching.length });
  }
}
...
console.log(
  `PDF_REMAINING_PRODUCTION_OK groups=${result.groups.length} clubProducts=${result.totalActiveProducts}`,
);
