...
const checks = [
  ...
  ["dimensões", migration.includes("length_cm numeric(10, 2)") && migration.includes("width_cm numeric(10, 2)") && migration.includes("height_cm numeric(10, 2)")],
  ["categorias normalizadas", migration.includes("CREATE TABLE IF NOT EXISTS public.categories") && migration.includes("CREATE TABLE IF NOT EXISTS public.product_categories")],
  ["opções de produto", migration.includes("CREATE TABLE IF NOT EXISTS public.product_options")],
  ["valores de opção", migration.includes("CREATE TABLE IF NOT EXISTS public.product_option_values")],
  ["variantes", migration.includes("CREATE TABLE IF NOT EXISTS public.product_variants")],
  ["combinações de variante", migration.includes("CREATE TABLE IF NOT EXISTS public.product_variant_values")],
  ["estoque por variante", migration.includes("stock_quantity integer NOT NULL DEFAULT 0")],
  ["estoque agregado", migration.includes("CREATE OR REPLACE FUNCTION public.refresh_product_stock")],
  ["imagem opcional", migration.includes("ALTER COLUMN image_url DROP NOT NULL")],
  ["sem tabela de imagens antecipada", !migration.includes("CREATE TABLE IF NOT EXISTS public.product_images") && !migration.includes("CREATE TABLE public.product_images")],
  ["tipos de categorias", types.includes("categories: {")],
  ["tipos de variantes", types.includes("product_variants: {")],
  ["tipos de status", types.includes('product_status: "draft" | "active" | "inactive" | "archived"')],
