...
async function runCase(browser, testCase, viewport, suffix) {
  ...
  page.on("response", (response) => {
    if ((response.url().startsWith(BASE) || response.url().includes("storefront_product_detail_v2")) && response.status() >= 400) {
  ...
  check(`${label}: produto tem default real`, Boolean(defaultVariant?.is_default));
  ...
  for (let index = 0; index < variants.length; index += 1) {
    ...
    check(`${label}: título ${text} é somente texto`, (await title.evaluate((node) => node.tagName)) === "P" && (await title.evaluate((node) => node.tabIndex)) === -1);
    check(`${label}: preview ${text} é button`, (await preview.evaluate((node) => node.tagName)) === "BUTTON");
    check(`${label}: preview ${text} tem aria-label`, (await preview.getAttribute("aria-label")) === `Selecionar variação ${text}`);
    check(`${label}: preview ${text} usa imagem da própria variante`, (await preview.getAttribute("data-variant-preview-image-id")) === expectedPrimary, `expected=${expectedPrimary}`);
    const img = page.locator(`[data-variant-preview-image="${variant.id}"]`);
    check(`${label}: preview ${text} carregou`, (await img.count()) === 1 && (await img.evaluate((node) => node.complete && node.naturalWidth > 0)));
  }

  check(`${label}: default abre selecionado`, (await selectedId(page)) === defaultVariant.id, `expected=${defaultVariant.id}`);
  ...
  check(`${label}: default tem destaque vermelho`, (await defaultPreview.getAttribute("class"))?.includes("border-red-600") === true);
  const expectedDefaultImage = sortedVariantImages(detail, defaultVariant.id)[0]?.id ?? null;
  check(`${label}: imagem principal corresponde ao default`, (await page.locator("[data-active-image-id]").getAttribute("data-active-image-id")) === expectedDefaultImage, `expected=${expectedDefaultImage}`);
  check(`${label}: preço do default preservado`, (await shownPrice(page)).includes(currency.format(effectivePrice(detail, defaultVariant))));
  ...
  await page.waitForFunction((id) => document.querySelector(`[data-variant-preview="${id}"]`)?.getAttribute("aria-pressed") === "true", target.id);
  check(`${label}: clicar no preview seleciona`, (await selectedId(page)) === target.id);
  const expectedTargetImage = sortedVariantImages(detail, target.id)[0]?.id ?? null;
  check(`${label}: imagem principal muda com a variante`, (await page.locator("[data-active-image-id]").getAttribute("data-active-image-id")) === expectedTargetImage, `expected=${expectedTargetImage}`);
  check(`${label}: preço da variante continua sincronizado`, (await shownPrice(page)).includes(currency.format(effectivePrice(detail, target))));

  if (viewport.width >= 640) {
    const galleryButtons = page.locator(`section[aria-label^="Galeria de"] button[aria-label^="Ver imagem de"]`);
    check(`${label}: galeria corresponde à variante selecionada`, (await galleryButtons.count()) === expectedGalleryImages(detail, target.id).length, `dom=${await galleryButtons.count()} expected=${expectedGalleryImages(detail, target.id).length}`);
  }

  await defaultPreview.focus();
  await page.keyboard.press("Enter");
  await page.waitForFunction((id) => document.querySelector(`[data-variant-preview="${id}"]`)?.getAttribute("aria-pressed") === "true", defaultVariant.id);
  check(`${label}: teclado seleciona preview`, (await selectedId(page)) === defaultVariant.id);
  ...
  if (viewport.width <= 390) {
    ...
    check(`${label}: alvo mobile confortável`, Boolean(box && box.width >= 96 && box.height >= 96), box ? JSON.stringify(box) : "sem box");
    check(`${label}: scrollbar visual oculta`, geometry.scrollbarWidth === "none", JSON.stringify(geometry));
  }

  const relevantConsole = consoleErrors.filter((text) => !/favicon|ResizeObserver|third-party cookie/i.test(text));
  check(`${label}: sem erro relevante de console`, relevantConsole.length === 0, relevantConsole.join(" | "));
  check(`${label}: sem 4xx/5xx/falha de rede relevante`, relevantNetwork.length === 0, relevantNetwork.join(" | "));
  const broken = await page.locator("img").evaluateAll((images) => images.filter((img) => img.complete && img.naturalWidth === 0).length);
...
try {
  for (const testCase of CASES) {
    await runCase(browser, testCase, { width: 1440, height: 1000 }, "desktop-1440");
    await runCase(browser, testCase, { width: 390, height: 844 }, "mobile-390");
  }
} finally {
  await browser.close();
}
